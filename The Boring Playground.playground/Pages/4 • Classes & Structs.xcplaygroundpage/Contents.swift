...
/// This can be read as: For every item position of the array `presenters` until `presenters.count` is reached, assign the index/position to the variable `position` and then assign the value at the current position of the array to the variable `presenter` and then call the method ``greet`` onto that presenter instance.
...
/// Now let's see what displays `simplePersonA.name`, just out of curiosity:

print(simplePersonA.name)
